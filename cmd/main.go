...
func main() {
	...
	flag.StringVar(&configPath, "config-path", "", "The path to the operator configuration file.")
	...
	if configPath == "" {
		setupLog.Error(nil, "config-path flag is required")
		os.Exit(1)
	}

	cfg, err := config.LoadConfig(configPath)
	if err != nil {
		setupLog.Error(err, "unable to load configuration", "config-path", configPath)
		os.Exit(1)
	}

	setupLog.Info("configuration loaded successfully",
		"api-server", cfg.API.Server.URL,
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "bf92f89f.lissto.dev",
