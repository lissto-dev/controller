...
BUILD_DIR=bin
CONFIG_DIR=config/default-config.yaml
BINARY_NAME=manager
...
run: manifests generate fmt vet ## Run a controller from your host.
	go run ./cmd/main.go --config-path $(CONFIG_DIR)
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	...
	$(KUBECTL) create configmap operator-config --from-file=config.yaml=config/default-config.yaml --dry-run=client -o yaml | $(KUBECTL) apply -f -

.PHONY: apply-config
apply-config: ## Apply the default configuration as a ConfigMap.
	@echo "Applying default configuration..."
	$(KUBECTL) create configmap operator-config --from-file=config.yaml=config/default-config.yaml --dry-run=client -o yaml | $(KUBECTL) apply -f -
...
undeploy: kustomize ## Undeploy controller from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	...
	@echo "Removing operator configuration..."
	$(KUBECTL) delete configmap operator-config --ignore-not-found=$(ignore-not-found)
